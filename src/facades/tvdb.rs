...
pub fn alias_router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/series/{id}/extended", get(series_extended))
        .route("/series/{id}/episodes/{season_type}", get(series_episodes))
        .route("/movies/{id}", get(movie))
}
...
async fn search(State(state): State<Arc<AppState>>, Query(params): Query<Value>) -> Response {
    let Some(raw) = params.get("query").and_then(Value::as_str) else {
        return tvdb_failure(StatusCode::BAD_REQUEST, "query parameter is required");
    };
    let Some(query) = search::normalize_query(raw) else {
        return Json(json!({ "status": "success", "data": [] })).into_response();
    };
