...
and fanart.tv are **not** mounted under a category root — Kusaritoi consumes each
artwork provider through its own dialect facade (`/coverart`, `/fanart`) and
rewrites image URLs through the `/_ia` byte proxy.
